if (JSON.parse(localStorage.getItem("showDetails"))) {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    document.getElementsByClassName("list-group-item")[document.getElementsByClassName("list-group-item").length - 1].innerHTML +=
      arr[i].price + "&#8377";
    document.getElementsByTagName("img")[document.getElementsByTagName("img").length - 1].setAttribute("src", arr[i].showImage);
    
    // localStorage.setItem(document.getElementsByClassName("card-title")[document.getElementsByClassName("card-title").length-1].innerHTML,"");
  }
}
